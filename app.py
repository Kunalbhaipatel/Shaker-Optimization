...
# Optional calendar filter if 'Date' column is available
...
# Shaker image (shown for all users)
st.sidebar.image("Hyperpool_SideView_Compression1_LR-removebg-preview (1).png",
                 caption="Hyperpool Shaker Unit",
                 use_container_width=True)
...
with tabs[1]:
    ...
    try:
        ...
        st.subheader("🔍 Shaker Performance Drop-Off Flagging")
        try:
            flag_threshold = -10  # example for poor data range
            drop_df = plot_df[plot_df['SHAKER #3 (PERCENT)'] < flag_threshold]
            fig_flag = go.Figure()
            fig_flag.add_trace(go.Scatter(x=plot_df['Timestamp'], y=plot_df['SHAKER #3 (PERCENT)'], mode='lines', name='SHAKER #3'))
            ...
            st.warning(f"Flag graph error: {e}")
